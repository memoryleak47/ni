        ...
        for (&fid, _) in ordered_map_iter(self.fns.iter()) {
            ...
            for (&bid, _) in ordered_map_iter(self.fns[&fid].blocks.iter()) {
                ...
                for st in &self.fns[&fid].blocks[&bid] {
					if SIMPLIFY && let Statement::Compute(n, expr) = st && let Some(x) = const_str(expr, &constmap) {
						constmap.insert(*n, x);
					} else {
...
fn display_stmt(stmt: &Statement, f: &mut Formatter<'_>, constmap: &Map<Node, String>) -> fmt::Result {
	write!(f, "    ")?;
	...
	match stmt {
		...
		Store(t, i, n) => {
			write!(f, "{}[{}] <- {}", node_string(*t, constmap), node_string(*i, constmap), node_string(*n, constmap))?;
		}
		...
		FnCall(n, t) => write!(f, "{}({})", node_string(*n, constmap), node_string(*t, constmap))?,
...
fn display_expr(expr: &Expr, f: &mut Formatter<'_>, constmap: &Map<Node, String>) -> fmt::Result {
	use Expr::*;
	match expr {
		Index(t, i) => write!(f, "{}[{}]", node_string(*t, constmap), node_string(*i, constmap))?,
		Arg => write!(f, "arg")?,
		NewTable => write!(f, "{{}}")?,
		Function(fid) => write!(f, "{}", fn_id_string(*fid))?,
		Type(v) => write!(f, "type({})", node_string(*v, constmap))?,
		Next(v1, v2) => write!(f, "next({}, {})", node_string(*v1, constmap), node_string(*v2, constmap))?,
		BinOp(kind, l, r) => write!(f, "{} {} {}", node_string(*l, constmap), kind, node_string(*r, constmap))?,
...
pub fn node_string(n: Node, constmap: &Map<Node, String>) -> String {
	constmap.get(&n).cloned().unwrap_or_else(|| format!("n{}", n))
}
